	...
	class easy_author_avatar_image {
            ...
            if ( $easy_author_avatar_image_option_set && $easy_author_avatar_image_roll ) {				
				...
				wp_localize_script(
					...
					array(
                        ...
                        '_default_image_url' => esc_attr( $avatar_url ),                        
                        )
				);
			}
		}
                ...
                <table>
                    <tbody>
                        ...
                        <?php  
                            ...
                            $roles = $wp_roles->roles;
                            foreach ($roles as $key=>$value) { 
                            $role_name = str_replace(array('_', ' '), ' ', $key);
                        ?>
                            <tr>
                                <td>
                                    <label class="easy-author-avatar-image-switch">
                                        <input type="checkbox" class="checkbox"
                                            name="easy_author_avatar_image_option[_enable_role][<?php echo esc_attr($key); ?>]"
                                            value="<?php echo esc_attr($key); ?>"
                                            <?php echo ( isset( $easy_author_avatar_image_option['_enable_role'][$key] ) && ( $key == $easy_author_avatar_image_option['_enable_role'][$key] ) ) ? ' checked="checked"' : ''; ?> />
                                        <div class="easy-author-avatar-image-slider"></div>
                                    </label>
                                    <label for=""
                                        class="easy-author-avatar-image-switch-lable"><?php echo esc_attr( ucwords($role_name) ); ?></label>
                                </td>
                            </tr>
		...
		public function admin_author_img_upload( $user ) {
			...
			if( $easy_author_avatar_image_option_set &&  $easy_author_avatar_image_roll ) {
                ...
                $classAdd = 'easy-author-avatar-image-hide';
                if( $avatar ){
                    $avatar_url = wp_get_attachment_image_url( $avatar );                    
                }else{
                    $avatar_url = get_avatar_url($user->ID);                   
                    $button_class = isset( $classAdd ) ? $classAdd: "";
                } 
                
                ?>

                <div class="easy-author-avatar-image-upload-wrap">
                    <input type="hidden" id="easy-author-avatar-image-id" class="easy-author-avatar-image-input"
                        name="easy-author-avatar-image-id" value="<?php echo isset( $avatar ) ? esc_attr( $avatar ) : ''; ?>">
                        ...
                        <tbody>
                            <tr class="easy-author-avatar-image-user-profile-picture">
                                ...
                                <td class="easy-author-avatar-image-show-section">
                                    <img class="avatar avatar-96 photo easy-author-avatar-img" id="easy-author-avatar-image-custom"
                                        src="<?php echo isset( $avatar_url ) ? esc_url( $avatar_url ) : ''; ?>" width="96" height="96"
                                        alt="" />
                                </td>
                                <td>
                                        ...
                                        <?php
                                            if ( ! current_user_can( 'upload_files' ) ) { ?>
                                                ...
                                                <span class="description">
                                                    <?php esc_html_e( 'Choose an image from your computer:' ); ?>
                                                </span><br />
                                                <input type="file" name="easy-author-avatar-image-upload-computer"
                                                    id="easy-author-avatar-image-upload-computer" class="standard-text" />

                                        <?php } else { ?>

                                                <button type="button" class="button easy-author-avatar-image-upload"
                                                    id="easy-author-avatar-image-upload">
                                                    <?php echo _e( 'Choose from Media Library', 'easy-author-avatar-image' );?>
                                                </button>

                                        <?php } ?>
                                        
                                        <button type="button" id="easy-author-avatar-image-delete-btn"
                                            class="button easy-author-avatar-image-remove <?php echo esc_attr( $button_class ); ?>">
